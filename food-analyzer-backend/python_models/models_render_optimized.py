#!/usr/bin/env python3
"""
Render-Optimized Model Functions for Food Detection
Excludes BLIP model to fit within 512MB RAM limit
"""
...
# Model availability flags (BLIP disabled for Render)
MODEL_AVAILABILITY = {
    'yolo': False,
    'vit': False,
    'swin': False,
    'blip': False,  # Disabled for memory constraints
    'clip': False
}
...
def check_model_availability():
    ...
    # Check Transformers (excluding BLIP)
    try:
        ...
        print("✅ Transformers (ViT, CLIP) available", file=sys.stderr)
    except ImportError as e:
    ...
    # BLIP is intentionally disabled
    print("ℹ️ BLIP disabled for memory optimization", file=sys.stderr)
...
def load_model(model_type: str) -> Optional[Any]:
    ...
    try:
        ...
        if model_type == 'yolo' and MODEL_AVAILABILITY['yolo']:
            ...
            print("❌ BLIP model disabled for memory optimization", file=sys.stderr)
            return None
...
def detect_with_blip(image: Image.Image, model_dict) -> Dict[str, Any]:
    ...
    return {
        ...
        'error': 'BLIP model not available in Render-optimized version'
    }
