#!/usr/bin/env python3
"""
YOLO-Only Flask App for Food Detection
...
def health_check():
    ...
    return jsonify({
        ...
        'models_enabled': ['yolo'],
        'models_disabled': ['vit', 'swin', 'blip', 'clip'],
        'memory_usage': '~50-100MB total'
    })
...
def test_endpoint():
    ...
    return jsonify({
        ...
        'note': 'Only YOLO model enabled for maximum memory optimization'
    })
    ...
    try:
        ...
        if model_type not in MODEL_CACHE:
            ...
            if model is None:
                return jsonify({
                    'success': False, 
                    'error': f'Failed to load {model_type} model. Only YOLO is available.',
                    'available_models': [k for k, v in MODEL_AVAILABILITY.items() if v],
                    'note': 'Only YOLO model enabled in this version for memory optimization'
                }), 500
        ...
        if model_type == 'yolo':
            ...
            return jsonify({'success': False, 'error': f'Unsupported model type: {model_type}. Only YOLO is available.'}), 400
...
def list_models():
    ...
    return jsonify({
        'available_models': ['yolo'],  # Only YOLO
        'loaded_models': list(MODEL_CACHE.keys()),
        'model_availability': MODEL_AVAILABILITY,
        'note': 'Only YOLO model enabled for maximum memory optimization. All other models disabled.',
        'version': 'yolo-only',
        'memory_usage': '~50-100MB total'
    })
...
if __name__ == '__main__':
    try:
        ...
        print(f"Memory optimization: Only YOLO enabled")
